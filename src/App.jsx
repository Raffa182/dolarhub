...
// Reemplaza esto con tus credenciales reales si se borraron
...
// Inicialización segura (try-catch para evitar pantalla azul si falla config)
let app, auth, db;
try {
...
// --- 3. COMPONENTES UI (Expandidos para evitar errores) ---
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
            <div className="bg-slate-900 w-full max-w-sm rounded-2xl border border-slate-700 p-8 relative shadow-2xl">
                ...
                <h2 className="text-2xl font-bold text-white mb-2 text-center">{isRegister ? 'Registro' : 'Ingresar'}</h2>
                <form onSubmit={handleAuth} className="space-y-4">
                    <div className="relative">
                        <Mail className="absolute left-3 top-3 text-slate-500" size={16} />
                        <input type="email" placeholder="Email" className="w-full bg-slate-800 border border-slate-600 rounded-lg pl-10 p-2 text-white outline-none focus:border-blue-500" value={email} onChange={e => setEmail(e.target.value)} />
                    </div>
                    <div className="relative">
                        <Key className="absolute left-3 top-3 text-slate-500" size={16} />
                        <input type="password" placeholder="Contraseña" className="w-full bg-slate-800 border border-slate-600 rounded-lg pl-10 p-2 text-white outline-none focus:border-blue-500" value={pass} onChange={e => setPass(e.target.value)} />
                    </div>
                    {error && <p className="text-xs text-red-400 bg-red-400/10 p-2 rounded">{error}</p>}
                    <button className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-2 rounded-lg transition-colors">{isRegister ? 'Crear Cuenta' : 'Entrar'}</button>
                </form>
                <p className="text-xs text-slate-500 text-center mt-4 cursor-pointer hover:text-blue-400" onClick={() => setIsRegister(!isRegister)}>
                    {isRegister ? '¿Ya tienes cuenta? Entrar' : '¿No tienes cuenta? Regístrate'}
                </p>
            </div>
        </div>
    );
...
const UserProfileModal = ({ isOpen, onClose, user, userData }) => {
    ...
    return (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in zoom-in-95">
            <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-slate-700 p-6 relative shadow-2xl">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X size={20} /></button>
...
const PaymentModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in zoom-in-95">
            <div className="bg-slate-900 w-full max-w-md rounded-2xl border border-slate-700 p-6 relative text-center shadow-2xl">
                <button onClick={onClose} className="absolute top-4 right-4 text-slate-400 hover:text-white"><X size={20} /></button>
                <Crown size={48} className="text-yellow-400 mx-auto mb-4 drop-shadow-md" />
                <h2 className="text-xl font-bold text-white">Hazte Premium</h2>
                <p className="text-slate-400 text-sm mb-6">Transfiere <strong>${BANK_INFO.price}</strong> al alias:</p>
                <div className="bg-slate-800 p-4 rounded-xl border border-slate-600 mb-6 cursor-pointer hover:bg-slate-700 transition-colors" onClick={() => { navigator.clipboard.writeText(BANK_INFO.alias); alert('Alias Copiado') }}>
                    <p className="font-mono text-xl font-bold text-white tracking-widest">{BANK_INFO.alias}</p>
                    <p className="text-xs text-blue-400 mt-1">(Clic para copiar)</p>
                </div>
                <button onClick={() => { alert('Aviso enviado. Actívalo en el panel Admin.'); onClose() }} className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all">
...
// Visor de Academia Simple
const AcademyViewer = ({ onBack }) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in">
            {ACADEMY_ARTICLES.map(art => (
                <div key={art.id} className="bg-slate-800 border border-slate-700 p-6 rounded-xl hover:border-blue-500/50 hover:shadow-lg transition-all cursor-pointer group flex flex-col h-full">
...
// --- CALCULADORA DE CONVERSIÓN CEDEARS ---
    ...
    return (
        <div className="bg-slate-800 rounded-xl p-6 border border-slate-700 shadow-lg h-full flex flex-col">
            <div className="flex items-center gap-3 mb-6">
                ...
                <div><h3 className="font-bold text-white">Calculadora Conversión</h3><p className="text-xs text-slate-400">CEDEARs a Acciones USA</p></div>
            </div>

            <div className="space-y-6 flex-1">
                <div>
                    <label className="text-[10px] text-slate-400 uppercase font-bold mb-2 block">Empresa</label>
                    <div className="relative">
                        <Search size={16} className="absolute left-3 top-3 text-slate-500" />
                        <select className="w-full bg-slate-900 border border-slate-600 rounded-xl pl-10 pr-4 py-3 text-white text-sm outline-none focus:border-blue-500 appearance-none" value={selectedTicker} onChange={(e) => setSelectedTicker(e.target.value)}>
                            {cedears.map(c => <option key={c.s} value={c.s}>{c.s} - {c.n}</option>)}
                        </select>
                    </div>
                </div>

                <div>
                    <label className="text-[10px] text-slate-400 uppercase font-bold mb-2 block">Tengo (Cantidad)</label>
                    <input type="number" min="0" className="w-full bg-slate-900 border border-slate-600 rounded-xl p-3 text-white text-lg font-mono outline-none focus:border-blue-500" value={ownedCedears} onChange={(e) => setOwnedCedears(Math.max(0, parseInt(e.target.value) || 0))} />
                </div>

                <div className="bg-slate-900/50 rounded-xl p-5 border border-slate-700/50 space-y-4">
                    <div className="flex justify-between items-center">
                        <span className="text-sm text-slate-300">Ratio</span>
                        <span className="text-xs font-mono bg-slate-800 px-2 py-1 rounded text-blue-400 border border-slate-700">{ratio}:1</span>
                    </div>
                    <div className="text-center py-2 border-t border-b border-slate-700/50">
                        ...
                        <p className="text-xs text-slate-400 mt-1">Acciones en USA</p>
                    </div>
                    {missingForNext > 0 ? (
                        <div className="flex items-start gap-3 bg-yellow-500/10 p-3 rounded-lg border border-yellow-500/20">
                            ...
                            <div className="text-xs text-slate-300">Compra <strong className="text-white">{missingForNext} más</strong> para completar tu acción N° {nextFullShare}.</div>
                        </div>
                    ) : (
                        <div className="flex items-center gap-2 bg-green-500/10 p-3 rounded-lg border border-green-500/20 justify-center">
                            <CheckCircle size={18} className="text-green-500" />
                            <span className="text-xs font-bold text-green-400">¡Acciones Completas!</span>
                        </div>
                    )}
                </div>
            </div>
    ...
    return (
        <div className="max-w-4xl mx-auto">
            <div className="text-center mb-10">
                ...
                <p className="text-slate-400">Elige las herramientas que necesitas.</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                {/* Plan Free */}
                <div className="bg-slate-800 rounded-2xl border border-slate-700 p-8 flex flex-col relative overflow-hidden shadow-lg">
                    <div className="mb-6"><h3 className="text-xl font-bold text-white">Inicial</h3><p className="text-slate-400 text-sm mt-1">Para empezar.</p></div>
                    <div className="mb-6"><span className="text-4xl font-bold text-white">$0</span><span className="text-slate-500 text-sm"> /mes</span></div>
                    <ul className="space-y-4 mb-8 flex-1">
                        <li className="flex items-center gap-3 text-sm text-slate-300"><CheckCircle size={16} className="text-slate-500" /> Cotizaciones en vivo</li>
                        <li className="flex items-center gap-3 text-sm text-slate-300"><CheckCircle size={16} className="text-slate-500" /> Calculadora CEDEARs</li>
                        <li className="flex items-center gap-3 text-sm text-slate-300"><CheckCircle size={16} className="text-slate-500" /> 2 Alertas Telegram</li>
                        <li className="flex items-center gap-3 text-sm text-slate-500 line-through decoration-slate-600"><X size={16} /> Gráficos Históricos</li>
                    </ul>
                    <button className="w-full py-3 rounded-xl border border-slate-600 text-white font-bold hover:bg-slate-700 transition-colors">Tu Plan</button>
                </div>
                ...
                <div className="bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl border border-yellow-500/50 p-8 flex flex-col relative shadow-2xl shadow-yellow-900/10 transform md:-translate-y-4">
                    ...
                    <div className="absolute top-4 right-4"><span className="bg-yellow-500 text-black text-[10px] font-bold px-3 py-1 rounded-full uppercase tracking-wider">Recomendado</span></div>
                    <div className="mb-6"><h3 className="text-xl font-bold text-white flex items-center gap-2"><Crown size={20} className="text-yellow-400" /> Trader Pro</h3><p className="text-slate-400 text-sm mt-1">Nivel profesional.</p></div>
                    <div className="mb-6"><span className="text-4xl font-bold text-white">$5.000</span><span className="text-slate-500 text-sm"> /mes</span></div>
                    <ul className="space-y-4 mb-8 flex-1">
                        <li className="flex items-center gap-3 text-sm text-white"><CheckCircle size={16} className="text-yellow-500" /> <strong>Todo lo del plan Free</strong></li>
                        <li className="flex items-center gap-3 text-sm text-white"><CheckCircle size={16} className="text-yellow-500" /> Alertas Ilimitadas</li>
                        <li className="flex items-center gap-3 text-sm text-white"><CheckCircle size={16} className="text-yellow-500" /> Gráficos Históricos</li>
                        <li className="flex items-center gap-3 text-sm text-white"><CheckCircle size={16} className="text-yellow-500" /> Soporte WhatsApp</li>
                    </ul>
                    <button onClick={onSubscribe} className="w-full py-3 rounded-xl bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-bold hover:shadow-lg hover:shadow-orange-500/20 transition-all">Mejorar Ahora</button>
                </div>
            </div>
        </div>
    );
...
const TelegramWidget = ({ user, userData, onLogin, onUpgrade }) => {
    ...
    const addAlert = async () => {
        ...
        await addDoc(collection(db, 'alerts'), { userId: user.uid, asset: newAlert.asset, condition: newAlert.cond, targetPrice: parseFloat(newAlert.price), active: true, telegramChatId: "1790304803" });
        setIsAdding(false); setNewAlert({ asset: 'Blue', cond: '>', price: '' });
    };
                ...
                {view === 'market' && (
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-in fade-in">
                        ...
                        <div className="lg:col-span-4 space-y-6">
                            ...
                            <div onClick={() => setView('tools')} className="bg-gradient-to-r from-orange-900/20 to-slate-800 border border-orange-500/20 p-5 rounded-xl cursor-pointer hover:border-orange-500/50 transition-colors group"><div className="flex justify-between items-start mb-2"><div className="bg-orange-500/20 p-2 rounded-lg text-orange-400"><Calculator size={20} /></div><ChevronRight className="text-slate-600 group-hover:text-white" /></div><h3 className="font-bold text-white">Calculadora Conversión</h3><p className="text-xs text-slate-400 mt-1">CEDEARs a Acciones USA.</p></div>
                        </div>
                    </div>
                )}
